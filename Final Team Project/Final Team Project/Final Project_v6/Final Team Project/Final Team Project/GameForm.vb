...
Public Class GameForm
    Dim intDice1 As Integer     ' Dice
    Dim intPlayer1 As Integer   ' Player 1 place holder
    Dim intPlayer2 As Integer   ' Player 2 place holder
    Dim intPlayer3 As Integer   ' Player 3 place holder
    Dim intPlayer4 As Integer   ' Player 4 holder
    Dim rand As New Random      ' New random for the dice
    Dim intTimer As Integer     ' Timer
    Dim missedFile As StreamWriter  ' A file for uploading missed questions
    Dim readmissedFile As StreamReader  ' A file for reading the missed questions
    Dim intPlayerTurn As Integer        ' Holds which player's turn it is
    Dim intQuestionStatus As Integer = 0

    Private Sub tmrQTimer_Tick(sender As Object, e As EventArgs) Handles tmrQTimer.Tick
        ' Increment the short answer timer when enabled
        intTimer += 1
    End Sub
    ...
    Private Sub GameForm_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        If intSelectOrCreate = 0 Then
            ' Open the file you created/selected.
            readingFile = File.OpenText(strFileName1)
        ElseIf intSelectOrCreate = 1 Then
            readingFile = File.OpenText(strFileName)
        End If

        ' Create a file to contain the missed questions.
        missedFile = File.AppendText("E:\MissedQuestions.txt")

        myQuestions.intGameType = readingFile.ReadLine()
    End Sub
    ...
    Sub MultipleChoice()
        ...
        If intValidation = 0 Then
            ...
            ElseIf intPlayerTurn = 2 Then
                intPlayer2 += intDice1
            ElseIf intPlayerTurn = 3 Then
                intPlayer3 += intDice1
            ElseIf intPlayerTurn = 4 Then
                intPlayer4 += intDice1
            End If
        ElseIf intValidation = 1 Then
            ' Write the missed question to the file.
            missedFile.WriteLine(myQuestions.strQuestion)
            missedFile.WriteLine(myQuestions.strCorrect)
            missedFile.WriteLine(myQuestions.strWrong1)
            missedFile.WriteLine(myQuestions.strWrong2)
            missedFile.WriteLine(myQuestions.strWrong3)
        End If

        If readingFile.EndOfStream = True Then
            intQuestionStatus = 1
        End If
    End Sub
    ...
    Sub ShortAnswer()
        ...
        myQuestions.strQuestion1 = readingFile.ReadLine()
            myQuestions.strAnswer = readingFile.ReadLine()

            ' Enable the timer
            tmrQTimer.Enabled = True

            ' Display the question to the user and allow them to answer
            strAnswer = InputBox(myQuestions.strQuestion1)

            ' When the timer = 10, then disable it and show a message
            If intTimer = 10 Then
                tmrQTimer.Enabled = False
                MessageBox.Show("Time is up.")
            End If

            If tmrQTimer.Enabled = False Then
            ...
            If strAnswer = myQuestions.strAnswer Then
                ...
                ElseIf intPlayerTurn = 4 Then
                    intPlayer4 += intDice1
                End If
                ' If it's incorrect, write the missed question to the file
            Else
                MessageBox.Show("That is incorrect. You have to stay back.")
                missedFile.WriteLine(myQuestions.strQuestion1)
                missedFile.WriteLine(myQuestions.strAnswer)
            End If
    ...
    Sub DiceImage()
        ' Show the dice image according to the rolled number
        If intDice1 = 1 Then
    ...
    Sub Player1()
        ...
        ElseIf intPlayer1 >= 30 Then
            ...
            ' Create a file filled with the missed questions and print
            readmissedFile = File.OpenText("E:\MissedQuestions.txt")
            MessageBox.Show("Print your missed questions for review.")
            Print()
    ...
    Private Sub btnRoll_Click(sender As Object, e As EventArgs) Handles btnRoll.Click
        ...
        If intQuestionStatus = 0 Then
            ...
            ElseIf myQuestions.intGameType = 1 Then
                ...
                ShortAnswer()
            End If
        ElseIf intQuestionStatus = 1 Then
            MessageBox.Show("All the questions have been answered.")
            intQuestionStatus = 2
        End If

        If intQuestionStatus = 2 Then
            intPlayer1 += intDice1
        End If
    ...
    Private Sub PrintDocument1_PrintPage(sender As Object, e As Printing.PrintPageEventArgs) Handles PrintDocument1.PrintPage
        e.Graphics.DrawString(readmissedFile.ReadLine(), New Font("MS Sans Serif", 12, FontStyle.Regular), Brushes.Black, 100, 100)
    End Sub
End Class